...
import { ApiOutlined, AppstoreOutlined, AuditOutlined, FundOutlined, RobotOutlined, TeamOutlined, UserOutlined } from '@ant-design/icons'
...
export default function DashboardPage({ onNavigate }) {
  ...
  const cards = [
    ...
    { title: '交付物', value: data.counts.deliverables || 0, icon: <AppstoreOutlined />, color: '#0e7490', bg: '#ecfeff', page: 'audit' },
    { title: '自动化任务', value: data.counts.automation_jobs || 0, icon: <ApiOutlined />, color: '#9333ea', bg: '#faf5ff', page: 'audit' },
    { title: '累计 token', value: data.counts.total_tokens || 0, icon: <FundOutlined />, color: '#dc2626', bg: '#fef2f2', page: 'usage' },
  ]
  return <div><div className="page-heading"><div><Title level={2}>平台概览</Title><Text type="secondary">运行环境：{environmentLabels[data.environment] || data.environment || '-'} · 默认模型：{data.default_model || '-'}</Text></div><Button loading={loading} onClick={load}>刷新</Button></div><Row gutter={[16, 16]}>{cards.map((item) => <Col xs={24} sm={12} xl={6} key={item.title}><Card className="admin-card stat-card" hoverable onClick={() => onNavigate(item.page)}><Space size={16}><span className="stat-icon" style={{ color: item.color, background: item.bg }}>{item.icon}</span><Statistic title={item.title} value={item.value} /></Space></Card></Col>)}</Row><Card className="admin-card" title="运营控制" style={{ marginTop: 20 }}><Space wrap><Button onClick={() => onNavigate('users')}>管理账号</Button><Button onClick={() => onNavigate('workspaces')}>查看租户边界</Button><Button onClick={() => onNavigate('audit')}>查看审计轨迹</Button><Button icon={<FundOutlined />} onClick={() => onNavigate('usage')}>查看用量统计</Button><Button icon={<ApiOutlined />} onClick={() => onNavigate('mcp')}>探测 MCP 服务</Button></Space></Card></div>
}
