...
2. **知识库**：上传文本类文件或手动创建；问答时**混合检索**（关键词 + 可选向量余弦融合）引用，配置 `EMBEDDING_PROVIDER=ollama` + `EMBEDDING_MODEL=qwen3-embedding`（`ollama pull qwen3-embedding`）即启用本地 Qwen 向量召回，未配置时自动退回关键词
