...
function WorkspaceApp({ session, onLogout }) {
  ...
  const sideMenu = <Menu theme="dark" mode="inline" selectedKeys={[nav]} onClick={({ key }) => { setNav(key); setMobileNav(false) }} items={[{ key: 'chat', icon: <MessageOutlined />, label: 'AI 对话' }, { key: 'business', icon: <RobotOutlined />, label: '汇报智能体' }, { key: 'board', icon: <ProjectOutlined />, label: '项目看板' }, { key: 'work', icon: <AppstoreOutlined />, label: '工作模式' }, { key: 'team', icon: <TeamOutlined />, label: '团队成员' }]} />
