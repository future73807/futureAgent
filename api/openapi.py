...
知识库接口维护智能助手可以引用的工作区资料：手动创建文档或上传 UTF-8 文本类
文件，标题、正文的任何改动都会重建向量切块。智能助手回答时会检索命中的片段，
并在回复中用 `[1]` 这样的编号标注来源。

- 所有知识库操作都以工作区为边界，只读成员只能查看，不能写入。
- 删除文档会级联删除它的向量切块，避免已删除的内容继续被检索召回。
- 未启用向量化（`EMBEDDING_PROVIDER=off`）时检索退回纯关键词匹配，接口行为不变。
...
OPENAPI_TAGS = [
    ...
    {
        "name": "知识库",
        "description": "工作区知识库文档的创建、上传、修改与删除，每次写入都会重建检索用的向量切块。",
    },
    {
        "name": "受治理的任务执行",
...
OPERATION_DOCUMENTATION: dict[tuple[str, str], dict[str, str]] = {
    ...
    ("GET", "/api/v1/knowledge-bases"): _operation(
        "列出知识库文档", "返回当前工作区的知识库文档清单，不含向量切块明细。", "知识库"
    ),
    ("POST", "/api/v1/knowledge-bases"): _operation(
        "创建知识库文档", "手动创建知识库文档并重建向量切块；只读成员不能调用。", "知识库"
    ),
    ("POST", "/api/v1/knowledge-bases/upload"): _operation(
        "上传知识库文档", "上传 UTF-8 文本类文件（最大约 400 KB）作为知识库文档并重建切块。", "知识库"
    ),
    ("PATCH", "/api/v1/knowledge-bases/{kb_id}"): _operation(
        "更新知识库文档", "更新标题、说明或正文，并重建该文档的向量切块。", "知识库"
    ),
    ("DELETE", "/api/v1/knowledge-bases/{kb_id}"): _operation(
        "删除知识库文档", "删除文档并级联删除它的向量切块，需要工作区管理权限。", "知识库"
    ),
