...
function BoardPage({ projects, tasks, members, onRefresh, openTask, workspaceRole }) {
  ...
  return (
    <div className="page-shell">
      ...
      <Modal title="新建任务" open={taskOpen} onCancel={() => setTaskOpen(false)} onOk={() => form.submit()} okText="确认" cancelText="取消" destroyOnClose>
      ...
      <Modal title="新建项目" open={projectOpen} onCancel={() => setProjectOpen(false)} onOk={() => projectForm.submit()} okText="确认" cancelText="取消" destroyOnClose>
...
function TaskExecutionPanel({ taskId, plan, models, skills, canWrite, onPlanRefresh }) {
  ...
  return <Card className="work-results" title="AI 执行" extra={<Space><Button type="primary" icon={<RobotOutlined />} loading={running} disabled={!executable} onClick={() => execute()}>执行选中步骤</Button>{running && activeRunId && <Button danger icon={<StopOutlined />} onClick={() => cancelRun(activeRunId)}>取消</Button>}</Space>}>
    <Space direction="vertical" size="small" style={{ width: '100%' }}><Text type="secondary">AI 只会接收已批准任务、选中计划步骤和附件中的有限文本上下文；结果保存后必须由人工审核，不会自动通过步骤。</Text><Flex gap={8} wrap="wrap"><Select value={stepId || undefined} onChange={setStepId} style={{ minWidth: 200 }} placeholder="选择计划步骤" options={(plan?.steps || []).filter((step) => step.status !== 'done').map((step) => ({ value: step.id, label: `${stepStatusLabels[step.status] || step.status} · ${step.title}` }))} /><Select value={modelId || undefined} onChange={setModelId} style={{ minWidth: 180 }} options={models.map((item) => ({ value: item.id, label: `${item.id}${item.ready ? '' : '（未就绪）'}`, disabled: !item.ready }))} /><Select value={skillName || undefined} onChange={setSkillName} style={{ minWidth: 150 }} options={skills.map((item) => ({ value: item.name, label: skillDisplayName(item.name) }))} /></Flex>{liveOutput && <pre className="attachment-preview">{liveOutput}</pre>}{runItems.length ? <Tabs size="small" items={runItems} /> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="此任务尚无 AI 执行记录" />}</Space>
  </Card>
}
...
function WorkModePage({ tasks, members, models, skills, workspaceRole, profile, onRefresh }) {
  ...
  const approve = async () => {
    try { const data = await apiFetch(`/api/v1/tasks/${taskId}/plan/approve`, { method: 'POST' }); setPlan(data.plan); message.success('计划已批准，可开始执行') } catch (error) { message.error(readableError(error)) }
  }
  ...
  return (
    <div className="page-shell work-mode">
      ...
      {!tasks.length ? <Empty description="请先在项目看板中创建任务" /> : <>
      ...
      <Modal title="记录步骤结果" open={Boolean(evidenceStep)} onCancel={() => setEvidenceStep(null)} onOk={() => evidenceForm.submit()} okText="确认" cancelText="取消" destroyOnClose>
...
function WorkspaceApp({ session, onLogout }) {
  ...
  const newConversation = async () => {
    try { const data = await apiFetch('/api/v1/conversations', { method: 'POST', body: JSON.stringify({ title: '新对话' }), workspaceId }); setConversations((previous) => [data.conversation, ...previous]); setActiveConversationId(data.conversation.id); setNav('chat') } catch (error) { message.error(readableError(error)) }
  }
  const moveTask = async () => { await loadWorkspace() }
  const sideMenu = <Menu theme="dark" mode="inline" selectedKeys={[nav]} onClick={({ key }) => { setNav(key); setMobileNav(false) }} items={[{ key: 'chat', icon: <MessageOutlined />, label: 'AI 对话' }, { key: 'business', icon: <RobotOutlined />, label: '经营助手' }, { key: 'board', icon: <ProjectOutlined />, label: '项目看板' }, { key: 'work', icon: <AppstoreOutlined />, label: '工作模式' }, { key: 'team', icon: <TeamOutlined />, label: '团队成员' }]} />
  const layoutSider = <><div className="workspace-brand"><Avatar icon={<RobotOutlined />} className="brand-avatar" /><div><strong>futureAgent</strong><span>团队 AI 工作空间</span></div></div><Select value={workspaceId || undefined} onChange={setActiveWorkspaceId} className="workspace-select" options={workspaces.map((item) => ({ value: item.id, label: item.name }))} />{sideMenu}<div className="sider-bottom"><Tag color={workspace?.role === 'owner' ? 'gold' : 'blue'}>{roleLabels[workspace?.role] || '成员'}</Tag></div></>
  const content = nav === 'chat' ? <ChatPage conversations={conversations} activeConversation={activeConversation} messages={messages} models={models} skills={skills} onNewConversation={newConversation} onSelectConversation={setActiveConversationId} onRefresh={loadWorkspace} workspaceRole={workspace?.role} /> : nav === 'business' ? <BusinessAssistantsPage workspaceRole={workspace?.role} members={members} currentUserId={profile?.id} /> : nav === 'board' ? <BoardPage projects={projects} tasks={tasks} members={members} onRefresh={moveTask} openTask={(task) => setTaskDrawer(task)} workspaceRole={workspace?.role} /> : nav === 'work' ? <WorkModePage tasks={tasks} members={members} models={models} skills={skills} workspaceRole={workspace?.role} profile={profile} onRefresh={loadWorkspace} /> : <TeamPage workspace={workspace} members={members} workspaceRole={workspace?.role} onRefresh={loadWorkspace} />
  return <Layout className="workspace-layout">{screens.lg ? <Sider width={258} theme="dark" className="workspace-sider">{layoutSider}</Sider> : <Drawer placement="left" open={mobileNav} onClose={() => setMobileNav(false)} width={280} styles={{ body: { padding: 0, background: '#101a35' } }}>{layoutSider}</Drawer>}<Layout><Header className="workspace-header"><Space>{!screens.lg && <Button type="text" icon={<MenuOutlined />} onClick={() => setMobileNav(true)} />}<Badge status="success" text="工作区数据受保护" /></Space><Dropdown menu={{ items: [{ key: 'profile', label: profile?.email, disabled: true }, { type: 'divider' }, { key: 'logout', icon: <LogoutOutlined />, label: '退出登录', onClick: onLogout }] }}><Button type="text"><Space><Avatar size="small" icon={<UserOutlined />} />{profile?.display_name}</Space></Button></Dropdown></Header><Content className="workspace-content">{loading ? <div className="loading-page"><Spin size="large" /></div> : <Suspense fallback={<div className="loading-page"><Spin size="large" /></div>}>{content}</Suspense>}</Content></Layout><Drawer title="任务详情" open={Boolean(taskDrawer)} onClose={() => setTaskDrawer(null)} width={460}>{taskDrawer && <Space direction="vertical" size="middle" style={{ width: '100%' }}><Title level={4}>{taskDrawer.title}</Title><Paragraph>{taskDrawer.description || '暂无任务说明。'}</Paragraph><Descriptions bordered size="small" column={1}><Descriptions.Item label="状态"><Tag>{stepStatusLabels[taskDrawer.status] || taskDrawer.status}</Tag></Descriptions.Item><Descriptions.Item label="优先级"><Tag>{priorityLabels[taskDrawer.priority] || taskDrawer.priority}</Tag></Descriptions.Item><Descriptions.Item label="截止日期">{taskDrawer.due_date || '-'}</Descriptions.Item></Descriptions><Button type="primary" icon={<AppstoreOutlined />} onClick={() => { setNav('work'); setTaskDrawer(null) }}>在工作模式中打开</Button></Space>}</Drawer></Layout>
}
...
export function Root() {
  return <ConfigProvider locale={zhCN} theme={{ algorithm: theme.defaultAlgorithm, token: { colorPrimary: '#4263eb', colorInfo: '#4263eb', borderRadius: 16, colorBgLayout: '#f3f6fc', fontFamily: '"PingFang SC", "Microsoft YaHei", ui-sans-serif, system-ui, sans-serif' } }}><AntApp><App /></AntApp></ConfigProvider>
}
