 ...
 * 断言"真的取回结果"而不只是"调用了工具"）、MCP（两个服务同时挂载与工具执行）、
 * 技能（管理端新建 → 用户端使用）、创造模式（新建智能体 → 使用 → 删除），以及
 * 「规划 → 保存为工作计划 → 批准 → 自主执行」的完整闭环。
 *
 * 运行前提：
 *   - 用户端 5173、管理端 5174、API 经前端代理可达、两个本地 MCP 服务在跑；
 *   - 被测工作区里有一份**带缺陷的** python-demo（见下），否则"计划+自主闭环"
 *     那一步必然失败——智能体核实后发现无需改动，就不会出现写入类工具调用；
 *   - 真实模型：E2E_MODEL=<模型 id>（默认 local-mock，需另起替身模型 127.0.0.1:8010）。
 *
 * 每次验收前重置素材（幂等）：
 *   py scripts/seed_python_demo.py --workspace <工作区 id> --force
 *   # 再删掉上一轮智能体自己加的测试与 __pycache__，基线回到"3 条测试 1 条失败"
 *
 * 用法：cd frontend && E2E_MODEL=GLM-5.3-Flash E2E_RUN_TIMEOUT=600000 node e2e/modes-capabilities.mjs
 */
